...
from hunavis.utils import str_to_list_of_np, quaternion_from_euler
...
class PeopleVisualizer(Node):
    ...
    def _human_callback(self, msg):
        ...
        for agent in msg.agents:
            pose = np.array(
                [agent.position.position.x, agent.position.position.y, agent.yaw]
            )
        ...
        self.get_logger().debug(
            f"Publish {len(human_markers.markers)} humans to /human_markers"
        )
    ...
    def _goals_callback(self):
        ...
        self.get_logger().debug(
            f"Publish {len(goal_markers.markers)} goals to /goal_markers"
        )
    ...
    def _create_marker(
        ...
        if len(marker_pose) > 2 and marker_pose[2] is not None:
            qx, qy, qz, qw = quaternion_from_euler(roll=0, pitch=0, yaw=marker_pose[2])
